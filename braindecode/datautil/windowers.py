...
def _create_windows_from_events(
    ...
    #######################
    # Code added by Georgios Zoumpourlis, to crop Physionet dataset in 4-second epochs
    ...
    orig_time_vals = [el['orig_time'] for el in ds.raw.annotations]

    for trial_cnt in range(len(ds.raw.annotations)):
        if duration_vals[trial_cnt] != 4.0:
            duration_vals[trial_cnt] = 4.0

    try:
        assert( len(np.unique(duration_vals))==1 )
    except:
        print('Varying trial length. Quitting...')
        quit()

    # annotations_fixed = mne.Annotations(onset_vals, duration_vals, description_vals, orig_time_vals)
    # TODO : check how to store time values in annotations
